...
- F4-R7 `Done` Booker names are visible to signed-in members only (F1-R2); former members show as "Tidigare medlem". Resolved via `get_user_display_names` RPC on `/` for approved members and appended to each calendar block's label (`app/page.tsx`, `app/dashboard-client.tsx`); visitors and pending users get no names.
- F4-R8 Imported courses cannot be created, moved or cancelled here (F5-R6).
- F4-R9 **Former members:** when a membership is removed (F7-R1) or an account deleted (F2-R8), their bookings stay confirmed and keep blocking; only admins can move/cancel them. `booked_by`/`activity_log.actor_id` are `ON DELETE SET NULL`; admin views (`/admin/bookings`) render a null owner as "Tidigare medlem". Now also wired into the member-facing dashboard UI (F4-R7 name display).
...
- F5-R5 `Done` If the tenant has no course room set, sync still stores the imported occasions (room stays implicit/nullable); they show on the schedule with their course name suffixed "(Lokal ej vald)" (`app/page.tsx`); they block no bookings, since conflict checks only run against occasions in the tenant's course room (`app/dashboard-actions.ts`'s `loadImportedOccasions` returns `[]` without a `course_room_id`, and `lib/dans-se/sync.ts` only flags conflicts when `tenant.course_room_id` is set). Covered by `lib/dans-se/sync.test.ts`.
...
- F9-R2 `Done` `/admin/tenants` (super-admins only): list tenants (name, domains, #members, #pending requests, last dans.se sync, SMTP status); create tenant (name, slug, first domain); activate/deactivate tenant (F9-R12); manage domains (UI reminds that the domain must be added in Vercel and in Supabase Auth redirect URLs — domain management UI itself not built, simplification); appoint/remove tenant admins by email (if the person hasn't signed in yet, the admin membership is created on their first sign-in) — covered by F9-R13's invite/admin-toggle.
- F9-R3 `Done` A super-admin opens any tenant from `/admin/tenants/[slug]` via "Öppna som admin", which sets the `tenant` cookie server-side for that slug and redirects to `/admin` — bypassing the approval queue (`is_tenant_admin` already treats super-admins as admins in every tenant, so no membership row is created). Every super-admin change, including this switch, is written to `platform_audit_log` (F9-R13). **Note:** on a tenant's registered custom domain the `tenant` cookie is ignored — the domain always resolves the tenant (see `lib/supabase/middleware.ts`) — so a super-admin must open the tenant's own domain (or its `?tenant=` on a non-custom-domain host) to act there; the cookie switch only works on shared/preview hosts.
- F9-R12 `Done` **Go-live check:** a tenant can't be activated until its SMTP test (F9-R10) succeeds. Enforced server-side in `setTenantActive` (`/admin/tenants/[slug]/actions.ts`) regardless of what the UI sends; the activate button is also disabled client-side with a Swedish explanation when the SMTP test hasn't succeeded.
...
- F9-R6 `Partial` **General** (`/admin/settings`): name, timezone (default `Europe/Stockholm`), max days ahead for bookings (default 90), club logo upload (PNG/WebP only — SVG deliberately unsupported, stored-XSS risk via inline `<script>`/`on*` — ≤1MB, magic-byte-verified server-side, not just MIME type) stored directly in the DB (`tenants.logo_data bytea`, `tenants.logo_mime`, `tenants.logo_updated_at`) instead of Supabase Storage. Served publicly via `GET /logo/[slug]` (route handler reading the bytes with the service-role client, returning `Content-Type` from `logo_mime`, `X-Content-Type-Options: nosniff`, and a long-lived `Cache-Control` + `ETag` keyed on `logo_updated_at`); `getTenantLogoUrl` returns `/logo/[slug]?v=<logo_updated_at>` for cache-busting on re-upload. Upload/remove actions go through the service-role client after `requireRole("admin")`; the tenants row select used elsewhere in the app lists explicit columns (never `select("*")`) so `logo_data` isn't shipped to the browser except by the logo route itself. Rendered above "Bokningssystem" on auth pages and in the protected/start-page headers via the `Logo` component; falls back to the current Gåsasteget default images when unset. Plain-URL fallback field removed.
...
- F9-R8 `Partial` **Theme** (`/admin/settings`): a single "Tema" panel with a swatch + native colour picker + hex text field per colour (primary, secondary, text, gradient from/via/to — the fields most tenants actually change day-to-day); other `theme` jsonb keys (onPrimary, accent, background, surface, mutedText, headerFrom, headerTo) still exist and are left untouched by this panel. A row of preset-palette chips (Gåsasteget lila, Turkos, Skog, Solnedgång) fills all six fields at once; a live preview card (mini hero using the gradient, the tenant's logo or name, and a primary-coloured button) updates as fields change, before saving. "Återställ standard" clears `theme`/`bg_gradient_*` back to null (defaults). Saved to `tenants.theme` jsonb + `tenants.bg_gradient_from/via/to` (hex, `via`/`to` nullable — a solid colour when only `from` is set); hex validated server-side with the existing `isValidHex`. Applied as server-rendered CSS variables in the root layout (`:root{--color-primary:...}` etc., no flash of default colours); `--tenant-bg` drives the `hero-gradient` utility (start-page hero, auth layout background, protected header) in place of the old hard-coded gradient — defaults reproduce the current Gåsasteget look (`#2d284d` → `#4b4280` → `#9e97c4`). `--color-primary`/`--color-secondary` also drive the `purple-main`/`purple-light` Tailwind tokens. No contrast warning (skipped).
...
tenants(id, slug UNIQUE, name, active, timezone, max_days_ahead DEFAULT 90, logo_url,
        logo_data bytea NULL, logo_mime NULL, logo_updated_at NULL,
        dans_se_org NULL, course_room_id NULL, theme jsonb,
        bg_gradient_from NULL, bg_gradient_via NULL, bg_gradient_to NULL,
