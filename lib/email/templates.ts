...
export function magicLinkEmail(tenantName: string, link: string) {
  return {
    ...
    html: `<p>Hej!</p><p>Klicka på länken nedan för att logga in på <strong>${tenantName}</strong>:</p><p><a href="${link}">${link}</a></p><p>Länken är giltig en kort stund. Om du inte begärde detta kan du ignorera mejlet.</p>`,
...
export function newAccessRequestEmail(
  ...
  return {
    ...
    html: `<p>${requesterName} (${requesterEmail}) har begärt tillgång till ${tenantName}.</p><p><a href="${adminUrl}">Hantera förfrågan</a></p>`,
...
export function accessApprovedEmail(tenantName: string) {
  return {
    ...
    html: `<p>Din förfrågan hos ${tenantName} har godkänts. Du kan nu logga in och boka.</p>`,
...
export function accessDeniedEmail(tenantName: string, reason: string | null) {
  return {
    ...
    html: `<p>Din förfrågan hos ${tenantName} har nekats.${
