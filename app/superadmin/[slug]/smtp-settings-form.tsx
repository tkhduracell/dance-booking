...
import { saveSmtpSettings, sendSmtpTestEmail } from "./actions";
...
export function SmtpSettingsForm({
  ...
  async function handleSubmit(formData: FormData) {
    setError(null);
    setMessage(null);
    const result = await saveSmtpSettings(slug, formData);
  ...
  async function handleTestEmail() {
    ...
    setMessage(null);
    setTesting(true);
    const result = await sendSmtpTestEmail(slug, testTo);
    setTesting(false);
    if (result.error) setError(result.error);
    else {
      setMessage(result.message ?? null);
